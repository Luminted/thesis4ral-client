...
export const CardTable = () => {
  ...
  const renderedCards = cards.map((card) => <CardEntity key={card.entityId} {...card} />);

  const renderedDecks = decks.map((deck) => <DeckEntity key={deck.entityId} {...deck} />);
  ...
  const storeTableDOMInfo = useCallback(() => {
    ...
    if (tableElement) {
      ...
      dispatch(
        setVerticalScalingRatio({
          numerator: height,
          divisor: tableVirtualHeight,
        }),
      );
      dispatch(
        setHorizontalScalingRatio({
          numerator: width,
